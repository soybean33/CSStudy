...
* 이미지 스플리팅  
많은 이미지를 다운로드받게 되면 과부하가 걸리기 때문에 많은 이미지가 합쳐 있는 하나의 이미지를 다운로드 받고, background-image의 position을 이용하여 이미지를 표기  
* 코드 압축  
...
3. 서버 앞단에 HTTPS를 제공하는 CDN(Content Delivery Network)을 둠  

## 2.5.5 HTTP/3  
TCP 위에서 돌아가는 HTTP/2와는 달리 QUIC(Quick UDP Internet Connections)이라는 계층 위에서 돌아가며, UDP 기반으로 돌아감  
멀티플렉싱을 가지고 있으며 초기 연결 설정 시 지연 시간 감소라는 장점이 있음  

### 초기 연결 설정 시 지연 시간 감소  
통신을 시작할 때 번거로운 3-웨이 핸드셰이크 과정을 거치지 않아도 됨  
QUIC는 첫 연결 설정에 클라이언트가 서버에 어떤 신호를 한 번 주고, 서버도 거기에 응답하기만 하면 바로 본 통신을 시작할 수 있기 때문에 1-RTT만 소요  
QUIC는 전송한 패킷이 손실되었다면 수신 측에서 에러를 검출하고 수정하는 방식인 순방향 오류 수정 메커니즘(FEC, Forword Error Correction)이 적용되어 열약한 네트워크 환경에서도 낮은 패킷 손실률을 자랑  